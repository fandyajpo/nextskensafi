...
const TableTahunAjaran = React.memo(({ act, tahunAjaran }) => {
  const { state, fnr } = React.useContext(GlobalContext);

  const handlePageChange = React.useCallback(async (r, s) => {
    const rr = await getter([
      "api/prot/get",
      { params: { uri: "bo/tahun_ajaran", offset: (r - 1) * 5, limit: 5 } },
    ]);

    if (rr.status && rr.data?.all) {
      fnr({ type: "listTahunAjaran", payload: rr.data.all });
    }
  }, []);

  React.useEffect(() => {
    fnr({ type: "listTahunAjaran", payload: tahunAjaran });
  }, []);

  React.useEffect(() => {
    if (env == "development") {
      console.log("TableTahunAjaran rerender");
    }
  });
  ...
  return (
    <div className="w-full h-full">
      <DataTable
        ...
        data={state?.listTahunAjaran?.tahunAjaran}
        responsive={true}
...
TableTahunAjaran.displayName = "TableTahunAjaran";

const FormTahunAjaran = (props, ref) => {
  const { state, fnr } = React.useContext(GlobalContext);
  ...
  const onSubmit = React.useCallback(async (data) => {
    let r;
    sErrMsg("");
    data.uri = "bo/config/tahun_ajaran";
  ...
  React.useEffect(() => {
    if (env == "development") {
      console.log("FormTahunAjaran rerender");
    }
  });
  ...
  return (
    <>
      <Modal
        ...
        desc={`Tahun Ajaran ${getValues().nama}`}
        remover={remover}
      />
      ...
      <div className="h-14 bg-gray-50 flex justify-between px-4 py-3">
        <div className="text-sm font-semibold flex flex-col">
          <p className="h-full flex items-end">List Tahun Ajaran</p>
          <p className="h-full font-light text-xs">
            Update / Remove Tahun Ajaran.
          </p>
        </div>
...
FormTahunAjaran.displayName = "FormTahunAjaran";
const FormTahunAjaranRef = React.forwardRef(FormTahunAjaran, {});
const FormTahunAjaranComp = React.memo(FormTahunAjaranRef);
...
const TahunAjaran = ({ tahunAjaran }) => {
  ...
  React.useEffect(() => {
    if (env == "development") {
      console.log("Tahun Ajaran rerender");
    }
  });
  ...
  return (
    <>
      <div className="w-full h-fit rounded shadow flex flex-col gap-2 relative overflow-hidden">
        <FormTahunAjaranComp ref={updateData} />
        <TableTahunAjaran act={act} tahunAjaran={tahunAjaran} />
      </div>
    </>
  );
};
